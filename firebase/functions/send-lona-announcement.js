...
if (!fs.existsSync(serviceAccountPath)) {
    // Fallback: any linkedup firebase-adminsdk key in this folder
    const dir = __dirname;
    const files = fs.readdirSync(dir);
    const keyFile = files.find(f => f.startsWith('linkedup-c3e29-firebase-adminsdk-') && f.endsWith('.json'));
    if (keyFile) {
        serviceAccountPath = path.join(dir, keyFile);
    } else {
...
macOS version 1.6.7 is now available!

What's New:

Bug Fixes
1. More than 1 admin can connect fireflies api to a groupchat
2. App lag and Forever Loading fixed

New Features & Improvements
- Schedule Message Sending Feature.
- Prominent Overdue Action Items Identification.
