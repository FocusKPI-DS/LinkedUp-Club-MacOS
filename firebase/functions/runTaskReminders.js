 ...
 * Expects header X-Reminder-Secret to match process.env.REMINDER_SECRET (or env).
 */
exports.runTaskReminders = functions.https.onRequest(async (req, res) => {
  console.log("🔔 runTaskReminders: invoked but currently DISABLED by user request.");
  res.status(200).json({ disabled: true, message: "Task Reminders are currently disabled." });
  return;
