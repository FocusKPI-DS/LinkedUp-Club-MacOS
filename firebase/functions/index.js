...
// Scheduled messages: send at exact date/time (implementation in scheduledMessages.js)
...
// Voice message transcoding: auto-convert .webm to .m4a for iOS compatibility
exports.transcodeVoiceMessage = require('./transcodeVoiceMessage').transcodeVoiceMessage;

// External APIs
exports.receiveExternalData = require('./externalApi').receiveExternalData;

// Qurio Integration APIs
exports.getChatList = require('./externalApi').getChatList;
exports.getChatFiles = require('./externalApi').getChatFiles;
exports.getChatMembers = require('./externalApi').getChatMembers;
exports.getUserInfo = require('./externalApi').getUserInfo;
exports.pollQurioCalls = require('./externalApi').pollQurioCalls;
exports.sendQurioMessage = require('./externalApi').sendQurioMessage;
exports.sendPersonalMessage = require('./externalApi').sendPersonalMessage;
exports.uploadQurioFile = require('./externalApi').uploadQurioFile;
exports.fixQurioMessages = require('./externalApi').fixQurioMessages;
exports.deleteMessage = require('./externalApi').deleteMessage;
exports.createGroup = require('./externalApi').createGroup;
exports.updateGroup = require('./externalApi').updateGroup;
exports.addMembers = require('./externalApi').addMembers;
exports.removeMembers = require('./externalApi').removeMembers;
exports.deleteGroup = require('./externalApi').deleteGroup;

// Qurio → Lona Authentication
exports.generateCustomTokenForQurio = require('./generateCustomTokenForQurio').generateCustomTokenForQurio;

// Email change verification (server-side, bypasses unreliable client SDK)
exports.sendChangeEmailVerification = require('./changeEmail').sendChangeEmailVerification;
exports.syncEmailAfterVerification = require('./changeEmail').syncEmailAfterVerification;
...
console.log("FUNCTIONS FIRESTORE IS:", typeof functions.firestore, functions.firestore);

exports.sendPushNotificationsTrigger = functions.firestore.document(`${kPushNotificationsCollection}/{id}`)
...
exports.sendUserPushNotificationsTrigger = functions.firestore.document(`${kUserPushNotificationsCollection}/{id}`)
  .onCreate(async (snapshot, _) => {
    try {
      // Ignore scheduled push notifications on create
      const scheduledTime = snapshot.data().scheduled_time || "";
...
exports.sendReactionNotificationTrigger = functions.firestore.document('chats/{chatId}/messages/{messageId}')
  .onUpdate(async (change, context) => {
    try {
      ...
      for (const [reactingUserId, emojis] of Object.entries(newReactions)) {
        ...
        // Check if message author has notifications enabled (global switch)
        try {
          const authorDoc = await firestore.doc(messageAuthorRef.path).get();
          if (authorDoc.exists) {
            const authorData = authorDoc.data();
            // Global switch: if notifications_enabled is false, skip all notifications
            if (authorData.notifications_enabled === false) {
              console.log(`⏭️ Skipping reaction notification for ${messageAuthorRef.path}: notifications_enabled is false`);
              continue;
            }
            // Per-type switch: check new_message_enabled
            if (authorData.new_message_enabled === false) {
              console.log(`⏭️ Skipping reaction notification for ${messageAuthorRef.path}: new_message_enabled is false`);
              continue;
            }
          }
        } catch (e) {
          console.log('Error checking notification preference:', e);
          // Fail-open: send notification if we can't check
        }
...
exports.sendMessageNotificationTrigger = functions
  ...
  .onCreate(async (snapshot, context) => {
    try {
      ...
      // Skip lona-service-chat (it has its own trigger)
      if (chatId === 'lona-service-chat') {
        return;
      }
      ...
      // Skip AI/bot messages to avoid notification spam
      if (messageData.sender_type === 'ai') {
        return;
      }
      ...
      // Filter out members who have disabled message notifications
      const enabledMembers = [];
      for (const member of otherMembers) {
        try {
          const memberPath = member.path || (member.id ? `users/${member.id}` : null);
          if (memberPath) {
            const memberDoc = await firestore.doc(memberPath).get();
            if (memberDoc.exists) {
              const memberData = memberDoc.data();
              // Global switch: if notifications_enabled is false, skip all notifications
              if (memberData.notifications_enabled === false) {
                console.log(`⏭️ Skipping message notification for ${memberPath}: notifications_enabled is false`);
                continue;
              }
              // Per-type switch: check new_message_enabled
              if (memberData.new_message_enabled === false) {
                console.log(`⏭️ Skipping message notification for ${memberPath}: new_message_enabled is false`);
                continue;
              }
            }
          }
          ...
          // Fail-open: include member if we can't check their preference
          enabledMembers.push(member);
        }
      }

      if (enabledMembers.length === 0) {
        console.log('No members with message notifications enabled');
        return;
      }
      ...
      if (isGroup && chatData.title) {
        ...
        notificationText = `${senderName}: ${notificationText}`;
      }
      ...
      // Get sender photo for notification
      let senderPhoto = "";
      try {
        const senderDocRef = senderRef.path ? firestore.doc(senderRef.path) : firestore.doc(`users/${senderRef.id}`);
        const senderDoc = await senderDocRef.get();
        if (senderDoc.exists) {
          senderPhoto = senderDoc.data().photo_url || "";
        }
      } catch (e) {
        // ignore
      }
      ...
      const notificationData = {
        ...
        notification_image_url: senderPhoto,
        notification_sound: "default",
        user_refs: enabledMembers.map(member => member.path).join(','),
        initial_page_name: "ChatDetail",
        parameter_data: JSON.stringify({
          chatDoc: `chats/${chatId}`,
        }),
        sender: senderRef,
        timestamp: admin.firestore.FieldValue.serverTimestamp(),
      };
      ...
      await firestore.collection(kUserPushNotificationsCollection).add(notificationData);
      console.log(`✅ Message notification sent for chat ${chatId}: "${notificationText.substring(0, 50)}..."`);
...
exports.sendLonaServiceMessageTrigger = functions.firestore.document('chats/lona-service-chat/messages/{messageId}')
  .onCreate(async (snapshot, context) => {
    try {
      const messageData = snapshot.data();
...
// ==========================================
// AUTO-UPDATE last_message ON ANY MESSAGE CREATION
// ==========================================
// This trigger ensures that the chat document's last_message fields are always
// up-to-date, even if the client fails to update them (network issues, race
// conditions, or buggy code paths).
exports.updateChatLastMessageTrigger = functions.firestore.document('chats/{chatId}/messages/{messageId}')
  .onCreate(async (snapshot, context) => {
    try {
      const chatId = context.params.chatId;
      ...
      const createdAt = messageData.created_at || admin.firestore.FieldValue.serverTimestamp();
      ...
      if (!previewText || previewText.trim() === '') {
        ...
        else if (type === 'video' || type === 'Video') previewText = '🎥 Video';
        else if (type === 'file' || type === 'File') previewText = '📎 File';
        else if (type === 'voice' || type === 'Voice') previewText = '🎵 Audio';
        else previewText = '';
      }
      ...
      // Only update if this message is actually newer than the current last_message
      const chatDoc = await chatRef.get();
      if (chatDoc.exists) {
        const chatData = chatDoc.data();
        const currentLastMessageAt = chatData.last_message_at;

        // If the chat already has a newer last_message_at, skip this update
        // This prevents race conditions where an older message's trigger fires after a newer one
        if (currentLastMessageAt && createdAt && currentLastMessageAt.toDate && createdAt.toDate) {
          try {
            if (currentLastMessageAt.toDate().getTime() > createdAt.toDate().getTime()) {
              console.log(`⏭️ Skipping update for chat ${chatId}: current last_message is newer`);
              return;
            }
          } catch (e) {
            // If timestamp comparison fails, proceed with update
          }
        }
      }
      ...
      if (senderRef) {
        updateData.last_message_sent = senderRef;
        // Reset last_message_seen to only include the sender
        // so other members see the unread badge (blue dot)
        updateData.last_message_seen = [senderRef];
      }

      await chatRef.update(updateData);
      console.log(`✅ Updated last_message for chat ${chatId}: "${previewText.substring(0, 50)}..."`);

    } catch (e) {
      console.log(`❌ Error updating chat last_message: ${e}`);
    }
  });

// Trigger for connection request notifications
exports.sendConnectionRequestNotificationTrigger = functions.firestore.document('users/{userId}')
  .onUpdate(async (change, context) => {
    try {
      ...
      // Global switch: if notifications_enabled is false, skip all notifications
      if (recipientData.notifications_enabled === false) {
        console.log(`⏭️ Skipping connection request notification for ${recipientUserId}: notifications_enabled is false`);
        return;
      }
      // Per-type switch: check connection_requests_enabled
      if (recipientData.connection_requests_enabled === false) {
        console.log(`⏭️ Skipping connection request notification for ${recipientUserId}: connection_requests_enabled is false`);
        return;
      }
...
exports.sendConnectionAcceptedNotificationTrigger = functions.firestore.document('users/{userId}')
  .onUpdate(async (change, context) => {
    try {
      const beforeData = change.before.data();
      const afterData = change.after.data();
      ...
      for (const friendId of newFriendIds) {
        ...
        try {
          ...
          const recipientDoc = await validRecipientRef.get();
          if (recipientDoc.exists) {
            const recipientData = recipientDoc.data();
            // Global switch
            if (recipientData.notifications_enabled === false) {
              console.log(`⏭️ Skipping connection accepted notification for ${friendId}: notifications_enabled is false`);
              continue;
            }
            // Per-type switch
            if (recipientData.connection_requests_enabled === false) {
              console.log(`⏭️ Skipping connection accepted notification for ${friendId}: connection_requests_enabled is false`);
              continue;
            }
          }
  ...
  if (userRefsStr) {
    for (var userRef of userRefs) {
      ...
      userTokens.docs.forEach((token) => {
        ...
        // Per-device notification control: skip tokens with notifications_enabled === false
        if (tokenData.notifications_enabled === false) {
          console.log(`⏭️ Skipping token (device: ${tokenData.device_type || 'unknown'}): notifications disabled for this device`);
          return;
        }
...
// ==========================================
exports.processAIMention = functions.https.onCall(async (data, context) => {
...
// ==========================================
exports.dailySummary = functions.https.onCall(async (data, context) => {
  // Check authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
...
// ==========================================
exports.InGroupSummer = functions.pubsub
...
// One-time backfill: update last_message for all chats based on their latest message
// DELETE THIS AFTER RUNNING ONCE
exports.backfillChatLastMessage = functions.https.onRequest(
  require('./backfillLastMessage').backfillChatLastMessage
);

// Unread Message Reminders
const unreadReminders = require('./unreadReminders');
exports.scheduleUnreadReminder = unreadReminders.scheduleUnreadReminder;
exports.clearUnreadReminder = unreadReminders.clearUnreadReminder;
exports.processUnreadReminders = unreadReminders.processUnreadReminders;