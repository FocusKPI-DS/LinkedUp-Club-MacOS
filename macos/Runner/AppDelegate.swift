...
class AppDelegate: FlutterAppDelegate {
  ...
  // MARK: - Pasteboard (Clipboard) Access Channel
  // Provides native access to NSPasteboard for pasting images/files into chat
  private func setupPasteboardChannel() {
    DispatchQueue.main.async { [weak self] in
      guard let self = self,
            let controller = NSApplication.shared.windows.first?.contentViewController as? FlutterViewController else {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
          self?.setupPasteboardChannel()
        }
        return
      }
      ...
      let channel = FlutterMethodChannel(
        name: "com.focuskpi.linkedup/pasteboard",
        binaryMessenger: messenger
      )
      ...
      channel.setMethodCallHandler { (call, result) in
        ...
        case "getImageData":
          // Returns PNG image bytes from pasteboard, or null if no image
          let pb = NSPasteboard.general
          
          // Try PNG first
          if let pngData = pb.data(forType: .png) {
            print("📋 [Pasteboard] Found PNG image data: \(pngData.count) bytes")
            result(FlutterStandardTypedData(bytes: pngData))
            return
          }
          
          // Try TIFF (screenshots are often TIFF) — convert to PNG
          if let tiffData = pb.data(forType: .tiff),
             let bitmapRep = NSBitmapImageRep(data: tiffData),
             let pngData = bitmapRep.representation(using: .png, properties: [:]) {
            print("📋 [Pasteboard] Found TIFF image, converted to PNG: \(pngData.count) bytes")
            result(FlutterStandardTypedData(bytes: pngData))
            return
          }
        ...
        case "getFileURLs":
          // Returns list of file path strings from pasteboard, or empty list
          let pb = NSPasteboard.general
          var paths: [String] = []
          ...
          print("📋 [Pasteboard] Found \(paths.count) file URLs")
          result(paths)
          
        case "hasImageOrFile":
          // Quick check: does pasteboard contain image data or file URLs?
          let pb = NSPasteboard.general
          let types = pb.types ?? []
          let hasImage = types.contains(.png) || types.contains(.tiff)
          let hasFile = types.contains(.fileURL)
          result(["hasImage": hasImage, "hasFile": hasFile])
      ...
      print("✅ [Pasteboard] Native pasteboard channel ready: com.focuskpi.linkedup/pasteboard")
    }
  }

  // MARK: - Notification Permission Method Channel
  // Handles 'com.lona.app/notifications' for Flutter Settings toggle
  private func setupNotificationChannel() {
    DispatchQueue.main.async { [weak self] in
      ...
      let messenger = controller.engine.binaryMessenger
      let channel = FlutterMethodChannel(
        name: "com.lona.app/notifications",
        binaryMessenger: messenger
      )
        ...
        case "getNotificationPermissionStatus":
          UNUserNotificationCenter.current().getNotificationSettings { settings in
            DispatchQueue.main.async {
              ...
              print("🔔 [Notifications] getPermissionStatus → \(isAuthorized) (raw: \(settings.authorizationStatus.rawValue))")
              result(isAuthorized)
            }
          }
        ...
        case "requestNotificationPermission":
          UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .badge, .sound]) { granted, error in
            DispatchQueue.main.async {
              ...
              print("🔔 [Notifications] Permission request result: \(granted)")
              if granted {
      ...
      print("✅ [Notifications] Method channel ready: com.lona.app/notifications")
    }
  }
  ...
  override func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    // Configure pasteboard access channel for paste support
    self.setupPasteboardChannel()
    // Configure notification permission channel for Settings toggle
    self.setupNotificationChannel()
  ...
  override func application(_ application: NSApplication, open urls: [URL]) {
    for url in urls {
      if url.absoluteString.contains("googleusercontent") || url.scheme?.hasPrefix("com.google") == true {
        // Activate app so Flutter plugin can receive the callback and complete signIn()
