  ...
  if (!kIsWeb && mediaSource == MediaSource.camera) {
    if (Platform.isIOS) {
      try {
        ...
        print('iOS permission check failed: $e');
        return null;
      }
  ...
  try {
    ...
    if (!kIsWeb && Platform.isMacOS && mediaSource == MediaSource.camera) {
      ...
      try {
        ...
        pickedMedia = await fallbackFuture;
      } catch (fallbackError) {
        print('Gallery fallback also failed: $fallbackError');
        return null;
      }
    } else {
...
String _getStoragePath(
  ...
  // Preserve original filename in storage path for better display.
  // Extract basename without extension, sanitize it for URL safety.
  final baseName = filePath.split('/').last.split('.').first;
  // Only include original name if it's meaningful (not empty or just whitespace)
  final safeName = baseName.replaceAll(RegExp(r'[^\w\-.]'), '_');
  if (safeName.isNotEmpty && safeName != '_') {
    return '$pathPrefix/${timestamp}${indexStr}_$safeName.$ext';
  }
