...
String _applyInlineFormat(String text, Map<String, dynamic> attributes) {
  ...
  if (attributes.containsKey('link') && attributes['link'] != null) {
    final linkUrl = attributes['link'].toString();
    // If the display text IS the URL itself, don't wrap it in [url](url) —
    // just output the plain URL. This avoids redundant markdown links
    // when Quill auto-links pasted URLs.
    if (res.trim() == linkUrl.trim()) {
      res = linkUrl;
    } else {
      res = '[$res]($linkUrl)';
    }
