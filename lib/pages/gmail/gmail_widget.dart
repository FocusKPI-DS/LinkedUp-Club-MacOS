...
class _GmailWidgetState extends State<GmailWidget> {
    ...
    try {
      ...
      if (cacheDoc.exists) {
        ...
        if (data != null && data['emails'] != null) {
          ...
          // Continue fetching more emails in background (up to 50 total) only when Gmail tab is visible
  ...
  Future<void> _loadMoreEmailsInBackground() async {
    ...
    // Don't run when this tab isn't visible (avoids competing with other pages)
    if (!mounted || (ModalRoute.of(context)?.isCurrent ?? false) == false) {
      return;
    }
