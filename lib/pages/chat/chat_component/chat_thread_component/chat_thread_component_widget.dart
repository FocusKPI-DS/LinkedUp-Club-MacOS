...
class _ChatThreadComponentWidgetState extends State<ChatThreadComponentWidget>
  ...
  bool _isComposing = false; // Track IME composing state reliably
  ...
  void initState() {
    ...
    // Track composing state via listener - this fires AFTER IME updates,
    // unlike onKey which fires BEFORE (making composing.isValid unreliable there)
    _model.messageTextController!.addListener(_onTextChanged);
      ...
      onKey: (node, event) {
        ...
        if (event is KeyDownEvent || event is KeyRepeatEvent) {
          ...
          if (event.logicalKey == LogicalKeyboardKey.enter ||
            ...
            // Use tracked composing state (reliable because listener fires after IME)
            // This prevents Enter from sending messages during CJK character selection
            if (_isComposing) {
              return KeyEventResult.ignored; // Let IME handle the Enter key
            }
  ...
  void dispose() {
    ...
    _model.messageTextController?.removeListener(_onTextChanged);
  ...
  /// Track IME composing state changes.
  /// This listener fires AFTER the IME has updated the composing range,
  /// making it reliable for CJK input detection (unlike onKey which fires before).
  void _onTextChanged() {
    final composing = _model.messageTextController?.value.composing;
    final nowComposing = composing != null && composing.isValid;
    if (nowComposing != _isComposing) {
      _isComposing = nowComposing;
    }
  }
  ...
  Future<void> _handleFilePicker() async {
    ...
    // Smart routing: detect file type by extension and route to proper model fields
    // so images/videos get proper previews instead of generic file icon
    const imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'heic'];
    const videoExtensions = ['mp4', 'mov', 'avi', 'mkv', 'webm', 'wmv', 'm4v'];

    final imageUrls = <String>[];
    final videoUrls = <String>[];
    final videoBytes = <Uint8List>[];
    final videoNames = <String>[];
    final genericFiles = <FFUploadedFile>[];
    final genericUrls = <String>[];

    for (int i = 0; i < selectedUploadedFiles.length && i < downloadUrls.length; i++) {
      final file = selectedUploadedFiles[i];
      ...
      if (imageExtensions.contains(ext)) {
        ...
        videoUrls.add(url);
        if (file.bytes != null) videoBytes.add(file.bytes!);
    ...
    // Route images to _model.images (shows thumbnail preview)
    if (imageUrls.isNotEmpty) {
      safeSetState(() {
        _model.images = [..._model.images, ...imageUrls];
      });
    }

    // Route first video to _model.selectedVideoFile (shows play icon preview)
    // Only one video at a time is supported
    if (videoUrls.isNotEmpty) {
      final videoStoragePath = videoUrls.first;
      safeSetState(() {
        _model.selectedVideoFile = SelectedFile(
          storagePath: videoStoragePath,
          bytes: videoBytes.isNotEmpty ? videoBytes.first : Uint8List(0),
        );
      });
    }

    // Route generic files to _model.file (shows doc icon)
    if (genericFiles.isNotEmpty) {
      if (genericFiles.length == 1) {
        safeSetState(() {
          _model.uploadedLocalFile_uploadDataFile = genericFiles.first;
          _model.uploadedFileUrl_uploadDataFile = genericUrls.first;
          _model.file = genericUrls.first;
        });
      } else {
        safeSetState(() {
          _model.uploadedLocalFile_uploadDataFile = genericFiles.first;
          _model.uploadedFileUrl_uploadDataFile = genericUrls.first;
          _model.file = genericUrls.first;
          _model.uploadedLocalFiles_uploadData = genericFiles;
          _model.uploadedFileUrls_uploadData = genericUrls;
        });
      }
    }
  ...
  Future<void> _handleDroppedFiles(List<XFile> droppedFiles) async {
    ...
    // If mixed media types, show info message
    if (imageFiles.isNotEmpty && (otherFiles.isNotEmpty || videoFiles.isNotEmpty)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Row(
            children: const [
              ...
              Expanded(
                  child: Text(
                      'Mixed file types detected. Each type will be processed separately.')),
            ],
          ),
    ...
    // Handle video files (one at a time - route to selectedVideoFile for play icon preview)
            ...
            SnackBar(
              content: Row(
                children: const [
                  ...
                  Expanded(
                      child:
                          Text('Only the first video was added. One video at a time.')),
                ],
              ),
              backgroundColor: Colors.blue,
            ),
                                      ...
                                      Padding(
                                        ...
                                        child: Stack(
                                          children: [
                                            GestureDetector(
                                              onTap: () async {
                                                // Open the video URL for preview
                                                final videoUrl = _model.selectedVideoFile?.storagePath ?? '';
