...
class ChatThreadComponentWidgetState extends State<ChatThreadComponentWidget>
  ...
  Future<void> _fetchMentionableUsers() async {
    ...
    final chatRef = widget.chatReference!;
    try {
      final members = chatRef.members;
      ...
      if (!mounted || widget.chatReference != chatRef) return;
      ...
      if (mounted && widget.chatReference == chatRef) {
        setState(() {
  ...
  /// Schedule a message to be sent at [scheduledAt]. Uploads attachments, builds
  /// message data, and writes to Firestore `scheduled_messages` for the Cloud
  /// Function to send at the exact time.
  Future<void> _scheduleMessage(DateTime scheduledAt) async {
    if (widget.chatReference == null || currentUserReference == null) return;
    if (_model.isSending == true) return;
    ...
    if (!hasText &&
        ...
        !hasVideo &&
        !hasPendingAttachments) {
      _showTopBanner('Add message content or attachments to schedule.');
      return;
    }

    if (scheduledAt.isBefore(DateTime.now())) {
      _showTopBanner('Schedule time must be in the future.', isError: true);
      return;
    }
    ...
    try {
      ...
      List<String> imagesList = [];
      if (hasImagesNow) imagesList = List<String>.from(_model.images);
      if (hasImageNow) imagesList.add(_model.image!);
      ...
      if (imagesList.isNotEmpty) messageData['images'] = imagesList;
      if (videoUrl != null && videoUrl.isNotEmpty) messageData['video'] = videoUrl;
      if (audioUrl != null && audioUrl.isNotEmpty) messageData['audio'] = audioUrl;
      if (hasFileNow) {
        messageData['attachment_url'] = _model.file;
        if (_model.uploadedLocalFile_uploadDataFile.name != null) {
          messageData['file_name'] = _model.uploadedLocalFile_uploadDataFile.name;
        }
      }
      if (_model.replyingToMessage != null) {
        messageData['reply_to'] = _model.replyingToMessage!.reference.id;
      ...
      safeSetState(() {});
      _showTopBanner('Message scheduled for ${_formatScheduleTime(scheduledAt)}');
    } catch (e) {
      _showTopBanner('Failed to schedule message: $e', isError: true);
    } finally {
      _model.isSending = false;
      safeSetState(() {});
    }
  ...
  String _formatScheduleTime(DateTime at) {
  ...
  /// Shows a top banner (green download-style) instead of a bottom snackbar.
  void _showTopBanner(String message, {bool isError = false}) {
    if (!mounted) return;
    final overlay = Overlay.of(context);
    late OverlayEntry overlayEntry;
    final backgroundColor =
        isError ? FlutterFlowTheme.of(context).error : Colors.green;
  ...
  /// Process a single dropped file and update UI immediately so preview appears as soon as it's read.
  Future<void> _processOneDroppedFile(XFile file) async {
    try {
  ...
  /// Handle files dropped via drag-and-drop. Processes files in parallel and
  /// updates the UI after each file so the first preview appears within ~1s.
  Future<void> _handleDroppedFiles(List<XFile> droppedFiles) async {
    ...
    // Process all files in parallel; each calls setState when done so previews appear incrementally.
    await Future.wait(
      droppedFiles.map((file) => _processOneDroppedFile(file)),
    );
  }
  ...
  Future<void> _filterGroupMembers(String query) async {
    ...
    try {
      ...
      if (mounted && widget.chatReference == chatRef && _model.showMentionOverlay) {
        _showMentionOverlay();
      }
    } catch (e) {
                          ...
                          builder: (context, snapshot) {
                            ...
                            return StreamBuilder<List<ReminderDigestsRecord>>(
                              ...
                              builder: (context, digestSnapshot) {
                                ...
                                if (digestSnapshot.hasError) {
                                  latestDigest = null;
                                } else if (digestSnapshot.hasData &&
