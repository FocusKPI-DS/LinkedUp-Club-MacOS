...
class _GroupChatDetailWidgetState extends State<GroupChatDetailWidget>
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      final chatDocToLoad = widget.chatDoc;
      if (chatDocToLoad == null) return;

      // Use document path to detect "same chat" across rebuilds (parent may pass new ChatsRecord instance)
      final chatPathToLoad = chatDocToLoad.reference.path;

      // Initialize text controllers with current values
      _model.groupNameController?.text = chatDocToLoad.title;
      // Don't initialize description if it's the same as group type
      final description = chatDocToLoad.description ?? '';
      ...
      try {
        ...
        if (eventRef != null) {
          ...
          _model.participant = <ParticipantRecord>[];
          _model.participants = [];
        }
        if (mounted && widget.chatDoc?.reference.path == chatPathToLoad) {
          safeSetState(() {});
        }
      } catch (e) {
        if (kDebugMode) {
          print('GroupChatDetail load error: $e');
        }
      } finally {
        if (mounted && (widget.chatDoc?.reference.path == chatPathToLoad)) {
    ...
    for (final msg in messages) {
      if (msg.attachmentUrl.isNotEmpty) {
        docs.add({
          ...
          'fileName':
              msg.content.isNotEmpty ? msg.content : 'file_${msg.reference.id}',
          'sender': msg.senderName.isNotEmpty ? msg.senderName : null,
  ...
  Widget _buildFirefliesSection(ChatsRecord? currentChatDoc) {
    final isAdmin = ChatHelpers.isGroupAdmin(currentChatDoc, currentUserReference);
          ...
          children: [
            Row(
              ...
              children: [
                ...
                TextButton(
                  ...
                  child: Text(
                    _model.firefliesShowManualInput ? 'Transcripts' : 'Manual Input?',
                  ...
                  onPressed: () async {
                    ...
                    if (key.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content:
                              Text('Please enter your Fireflies API key'),
                    ...
                    try {
                      ...
                      final transcriptsResult =
                        ...
                        limit: GroupChatDetailModel.firefliesPageSize,
                        skip: 0,
                      );
                      ...
                      _model.firefliesHasMore =
                          transcriptsResult.transcripts.length >=
                              GroupChatDetailModel.firefliesPageSize;
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
            ...
            if (hasApiKey || _model.firefliesShowManualInput) ...[
              ...
              if (_model.firefliesShowManualInput)
                _buildMeetingTranscriptsManualInput(currentChatDoc)
              else
  ...
  Widget _buildFirefliesContent(String chatId) {
    // Auto-load transcripts once when connected. Schedule only in post-frame to avoid setState during build.
    ...
    return _buildFirefliesTranscriptsList(chatId);
  }

  /// Manual text input shown inside Meeting Transcripts when "Manual Input?" is toggled.
  Widget _buildMeetingTranscriptsManualInput(ChatsRecord? chatDoc) {
    if (widget.chatDoc == null) return const SizedBox.shrink();
    if (chatDoc != null &&
        !_model.manualMeetingTranscriptionInitialized &&
      ...
      _model.manualMeetingTranscriptionController!.text =
          chatDoc.manualMeetingTranscription;
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      mainAxisSize: MainAxisSize.min,
      children: [
        ...
        FFButtonWidget(
          onPressed: () async {
            ...
            if (widget.chatDoc?.reference.id == null) return;
            safeSetState(() => _model.manualTranscriptSaving = true);
            try {
              ...
              if (context.mounted) {
                safeSetState(() => _model.manualTranscriptSaving = false);
  ...
  List<FirefliesTranscriptItem> _getFilteredFirefliesTranscripts() {
    final query =
  ...
  Widget _buildFirefliesTranscriptsList(String chatId) {
    final filtered = _getFilteredFirefliesTranscripts();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        TextField(
  ...
  Future<void> _removeUserAdmin(UsersRecord user) async {
    if (widget.chatDoc == null) return;
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // Content
              Text(
                'Are you sure you want to remove ${user.displayName} as admin? They will become a regular member.',
                style: const TextStyle(
                  fontFamily: 'Inter',
                  fontSize: 14.0,
                  fontWeight: FontWeight.normal,
    ...
    try {
      ...
      await _sendSystemMessage(
        '$userName removed ${user.displayName} as admin',
      );

      if (mounted) {
        _showSuccessDropdown('${user.displayName} is no longer admin');
      }
    } catch (e) {
      if (mounted) {
        _showErrorDropdown('Failed to remove admin');
      }
    }
  }
        ...
        body: SafeArea(
          ...
          child: StreamBuilder<ChatsRecord>(
            stream: widget.chatDoc != null
                ...
                : null,
            builder: (context, snapshot) {
              // Use the latest chat data from stream, or fallback to widget.chatDoc (including when stream errors)
              final currentChatDoc = snapshot.hasError || !snapshot.hasData
                  ? widget.chatDoc
                  : snapshot.data!;
                ...
                child: Stack(
                  children: [
                    _model.showMediaLinksDocs
                        ? Expanded(
                            child: _buildMediaLinksDocsView(),
                          )
                                              ...
                                              return Padding(
                                                ...
                                                child: Center(
                                                  child: Column(
                                                    mainAxisSize: MainAxisSize.min,
                                                    children: [
                                                      Icon(Icons.error_outline, size: 32, color: FlutterFlowTheme.of(context).error),
                                                      const SizedBox(height: 8),
                                                      Text('Couldn\'t load members', style: FlutterFlowTheme.of(context).bodySmall),
                                                    ],
                                                  ),
                                                ),
                                              );
                                                                              ...
                                                                              children: [
                                                                                      ...
                                                                                      children: [
                                                                                        Row(
                                                                                          ...
                                                                                          children: [
                                                                                            ...
                                                                                            else if (ChatHelpers.isGroupAdmin(widget.chatDoc, rowUsersRecord.reference))
                                                                                              Padding(
                                                                                                padding: const EdgeInsetsDirectional.only(start: 6.0),
                                                                                                child: Container(
                                                                                                  padding: const EdgeInsets.symmetric(horizontal: 6.0, vertical: 2.0),
                                                                                ...
                                                                                if (ChatHelpers.isGroupAdmin(currentChatDoc, currentUserReference) &&
                                                                                  ...
                                                                                  PopupMenuButton<String>(
                                                                                    ...
                                                                                    itemBuilder: (context) => [
                                                                                      // Owner-only: Make Admin (for non-admins)
                                                                                      if (ChatHelpers.isGroupOwner(currentChatDoc, currentUserReference) &&
                                                                                      ...
                                                                                      // Owner-only: Remove Admin (for admins who are not the owner)
                                                                                      if (ChatHelpers.isGroupOwner(currentChatDoc, currentUserReference) &&
                                                                                          ChatHelpers.isGroupAdmin(currentChatDoc, rowUsersRecord.reference) &&
                                                                                          !ChatHelpers.isGroupOwner(currentChatDoc, rowUsersRecord.reference))
                                                                                        PopupMenuItem<String>(
                                                                                          value: 'remove_admin',
                                                                                          child: Row(
                                                                                            children: [
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        child: StreamBuilder<
                                                                            UsersRecord>(
                                                                          stream:
                                                                              UsersRecord.getDocument(reportListsItem.reportedUser!),
                                                                          builder:
                                                                              (context, snapshot) {
                                                                            // Customize what your widget looks like when it's loading.
                                                                            if (!snapshot.hasData) {
