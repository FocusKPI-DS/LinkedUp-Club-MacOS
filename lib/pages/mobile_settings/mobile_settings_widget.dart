...
class _MobileSettingsWidgetState extends State<MobileSettingsWidget>
  ...
  Future<void> _loadNotificationSettings() async {
    ...
    bool deviceNotificationEnabled = true; // default to enabled
    ...
    // Then check device token's notifications_enabled field
    try {
      ...
      if (tokenDocRef != null) {
        ...
        deviceNotificationEnabled = data?['notifications_enabled'] ?? true;
      }
    } catch (e) {
      print('Error reading device token notification status: $e');
    }

    // Both system permission AND device preference must be true
    if (mounted) {
      setState(() {
        _pushNotificationsEnabled = systemPermissionGranted && deviceNotificationEnabled;
          ...
          return Container(
            ...
            child: GestureDetector(
              onTap: () {
                if (item['page'] == 'LogOut') {
                  _showLogoutDialog();
                } else if (item['page'] == 'API Keys') {
                  // Navigate directly to ProfileSettingsWidget with API Keys tab
                  Navigator.of(context).push(
                    CupertinoPageRoute(
                      builder: (context) => ProfileSettingsWidget(initialTab: SettingsTab.apiKeys),
                    ),
                  );
                } else {
                  setState(() {
  ...
  Future<void> _handlePushNotificationsToggle(bool newValue) async {
    if (newValue) {
      // Turning ON - Request system notification permission first
            ...
            if (mounted) {
              showCupertinoDialog(
                ...
                builder: (context) => CupertinoAlertDialog(
                  ...
                  content: Text(
                    'Please enable notifications in Settings > Notifications > Lona Club.',
                  ),
                  actions: [
                    CupertinoDialogAction(
                      child: Text('OK'),
      ...
      if (!permissionGranted) {
        // System permission not granted, don't update device token
        await _loadNotificationSettings();
        return;
      }

      // System permission granted — update device token doc
      setState(() {
      ...
      try {
        ...
        if (tokenDocRef != null) {
          ...
          print('✅ Notifications enabled for this device (iOS)');
        }
      } catch (e) {
        print('Error updating device token: $e');
      }
    } else {
      // Turning OFF — directly update device token doc (per-device control)
      setState(() {
      ...
      try {
        ...
        if (tokenDocRef != null) {
          ...
          print('✅ Notifications disabled for this device (iOS)');
        }
      } catch (e) {
        print('Error updating device token: $e');
      }
    ...
    return CupertinoPageScaffold(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            SizedBox(height: 24),
            // Chat Display - Font Size
            Padding(
              padding: EdgeInsets.symmetric(horizontal: 16),
              child: _buildIOSSection(
                header: 'CHAT DISPLAY',
                footer:
                    'Adjust the text size for chat messages. This applies to all conversations.',
                children: [
                  // Font size slider tile
                  Container(
              ...
              child: _buildIOSSection(
                ...
                children: [
                  _buildIOSListTile(
                    ...
                    onTap: () {
                      _showLanguagePicker(
                        ...
                        (lang) {
                          setState(() {
                            FFAppState().translateLanguage = lang;
                          });
                        },
                      );
                    },
                  ...
                  _buildIOSNotificationTile(
                    ...
                    subtitle: FFAppState().autoTranslate
            ...
            SizedBox(height: 24),
            // AI Translation Settings (Outgoing)
            Padding(
              padding: EdgeInsets.symmetric(horizontal: 16),
              child: _buildIOSSection(
                header: 'OUTGOING TRANSLATION',
                footer:
                    'Automatically translate your outgoing messages using free translation. Translation results may not be accurate — please use with caution.',
                children: [
                  _buildIOSNotificationTile(
                    title: 'Enable Outgoing Translation',
                    subtitle: FFAppState().aiTranslationEnabled
                        ? 'Enabled'
                        : 'Disabled',
                  ...
                  _buildIOSListTile(
                    ...
                    onTap: () {
                      _showLanguagePicker(
                        ...
                        (lang) {
                          setState(() {
                            FFAppState().aiTranslationTargetLanguage = lang;
                          });
                        },
                      );
                    },
