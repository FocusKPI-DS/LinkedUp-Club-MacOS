...
class ChatsRecord extends FirestoreRecord {
  ...
  // "is_pin" field — DEPRECATED, use pinned_by instead.
  bool? _isPin;
  bool get isPin => _isPin ?? false;
  bool hasIsPin() => _isPin != null;

  // "pinned_by" field — per-user pin: array of user DocumentReferences.
  List<DocumentReference>? _pinnedBy;
  List<DocumentReference> get pinnedBy => _pinnedBy ?? const [];
  bool hasPinnedBy() => _pinnedBy != null;

  /// Check whether a specific user has pinned this chat.
  bool isPinnedByUser(DocumentReference? userRef) {
    if (userRef == null) return false;
    return pinnedBy.any((ref) => ref.path == userRef.path);
  }
  ...
  // "admin_users" field — list of admin DocumentReferences (multi-admin support).
  List<DocumentReference>? _adminUsers;
  List<DocumentReference> get adminUsers => _adminUsers ?? const [];
  ...
  bool get hasActiveMeeting =>
      _activeMeeting != null && (_activeMeeting!['url'] as String?)?.isNotEmpty == true;

  String get activeMeetingUrl => (_activeMeeting?['url'] as String?) ?? '';
  DocumentReference? get activeMeetingStartedBy =>
      _activeMeeting?['started_by'] as DocumentReference?;
  DateTime? get activeMeetingStartedAt =>
      _activeMeeting?['started_at'] as DateTime?;
  List<DocumentReference> get activeMeetingParticipants =>
      (_activeMeeting?['participants'] as List<dynamic>?)
