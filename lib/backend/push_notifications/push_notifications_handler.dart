...
Future<void> handleNotificationNavigation(RemoteMessage message) async {
  ...
  try {
    ...
    if (!kIsWeb && Platform.isMacOS &&
      ...
      print('🖥️ macOS: Redirecting $initialPageName → DesktopChat');
        ...
        try {
          ...
          if (chatSnapshot.exists) {
            ...
            if (navigatorContext != null) {
              ...
              // After navigation, select the chat in ChatController
              Future.delayed(const Duration(milliseconds: 300), () {
                try {
                  final controller = ChatController.instanceOrNull;
                  if (controller != null) {
                    controller.selectChat(chatRecord);
                    print('✅ macOS: Selected chat ${chatRecord.reference.id} in DesktopChat');
                  } else {
                    print('⚠️ macOS: ChatController not available yet, storing pending chat');
                    ChatController.pendingNotificationChat = chatRecord;
                  }
                } catch (e) {
                  print('⚠️ macOS: Error selecting chat: $e');
                }
              });
              return;
            }
          ...
          print('❌ macOS: Error loading chat doc: $e');
        }
      }
      // Fallback: just go to DesktopChat tab
      final navigatorContext = appNavigatorKey.currentContext;
      if (navigatorContext != null) {
        navigatorContext.goNamed(
          '_initialize',
