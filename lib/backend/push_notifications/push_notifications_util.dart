...
Stream<UserTokenInfo> getFcmTokenStream(String userPath) =>
    Stream.value(!kIsWeb && (Platform.isIOS || Platform.isAndroid || Platform.isMacOS))
            ...
            (_) async {
              try {
                ...
                if (settings.authorizationStatus == AuthorizationStatus.authorized) {
                  // For macOS, we need to ensure APNS token is available first
                  if (Platform.isMacOS) {
                    ...
                    try {
                      ...
                      print('APNS token not available yet: $e');
                      return null;
                    }
                  }
    ...
    .map(
      (userTokenInfo) => makeCloudCall(
        'addFcmToken',
        {
          ...
          'deviceType': Platform.isIOS ? 'iOS' : Platform.isAndroid ? 'Android' : 'macOS',
        },
      ),
    );
